...
//--- Global variables -------
var verbList, userInput, input, output, historyTable, historyHeader;
...
function storeHistory(base, third, past, pastp, prep) {
  ...
  // Bind history table
  history = `<tr ondblclick="this.style.display = 'none'; refreshHistory();">
  <td>${base}</td>
  <td>${third}</td>
  <td>${past}</td>
  <td>${pastp}</td>
  ...
  history = "";
  // Show history output
  historyTable.innerHTML = currentHistory();
}
...
function currentHistory() {
  if (localStorage.getItem('history')) {
    ...
    historyHeader.innerHTML = tableHeader;
    return localStorage.getItem('history');
  } else {
    var empty = `<tr><td colspan="5"><div class="alert alert-warning m-0" role="alert">
